...
const CATEGORIES_CONFIG = [
    ...
    { id: "work", color: "bg-blue-100 text-blue-700", icon: "💼" },
    { id: "study", color: "bg-violet-100 text-violet-700", icon: "🎓" },
    { id: "shop", color: "bg-orange-100 text-orange-700", icon: "🛒" },
];
...
const createTodo = async (newItem: { title: string; date: string | null; category: string }) => {
    const user = await getUser();
    const { data, error } = await supabase.from("todos").insert([{ title: newItem.title, user_id: user.id, due_date: newItem.date ? new Date(newItem.date).toISOString() : null, category: newItem.category }]).select().single();
    if (error) throw error;
    return data;
};
...
function SwipeableTodoItem({ todo, toggle, remove, lang }: { todo: TodoItem, toggle: any, remove: any, lang: Lang }) {
    ...
    return (
        ...
        >
            <motion.div style={{ background }} className="absolute inset-0 rounded-2xl flex items-center justify-between px-6 z-0">
                <Check className="text-white font-bold" size={24} />
                <Trash2 className="text-white font-bold" size={24} />
            </motion.div>
            ...
            >
                ...
                <div className="flex-1 min-w-0 pt-0.5 pointer-events-none select-none">
                    <div className="flex items-center gap-2 mb-1 opacity-80">
                        <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider ${catConfig.color} bg-opacity-50`}>
                            {catLabel}
                        </span>
...
export function Todo() {
    ...
    // AI Generator (Dynamic Prompt based on Lang)
    const handleAiGenerate = async () => {
        ...
        try {
            ...
            const promptEn = `
                ...
                Goal: break down "${newTodo}" into 3-6 concrete steps.
                Format: Strict JSON array of strings in English.
                Example: ["Buy tickets", "Pack bag"]
            `;
            const promptRu = `
                Ты — планировщик задач.
                Цель: разбить задачу "${newTodo}" на 3-6 конкретных шагов.
                Формат: Строгий JSON массив строк на русском языке.
                Пример: ["Купить билеты", "Собрать сумку"]
            `;

            const prompt = lang === 'en' ? promptEn : promptRu;
            ...
            if (textAnswer) {
                ...
                for (const task of tasks) {
                    if (task && typeof task === 'string') {
                        await addMutation.mutateAsync({ title: task, date: selectedDate || null, category: selectedCategory });
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 pb-20 overflow-x-hidden">
            ...
            <div className={`relative mb-10 transition-all duration-300 ${aiMode ? "shadow-[0_0_40px_-10px_rgba(124,58,237,0.3)]" : "shadow-xl shadow-gray-100"}`}>
                <div className={`bg-white rounded-3xl p-1.5 pl-3 flex items-center gap-2 border transition-colors ${aiMode ? "border-purple-200 ring-2 ring-purple-50" : "border-transparent"}`}>

                    {/* Category Button */}
                    <div className="relative flex-shrink-0">
                        <button
                            onClick={() => setIsCategoryOpen(!isCategoryOpen)}
                            className={`w-10 h-10 rounded-2xl flex items-center justify-center text-lg transition-transform active:scale-95 ${currentCatConfig.color}`}
                        >
                    ...
                    <div className="flex items-center gap-1 pr-1 flex-shrink-0">
                        ...
                        <button
                            ...
                            className={`ml-1 w-12 h-12 rounded-2xl flex items-center justify-center transition-all active:scale-90 disabled:opacity-50 disabled:active:scale-100 ${
